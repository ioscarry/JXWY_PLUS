...
for data in ad_data:
    ...
    #        data['spread_index'],
    #        data['law_index'],
    #        data['venture_index']]  #交给协会的版本
    if count < 200:
        data['level_venture'] = '1'   # 排名前200的条目，分为1类
    elif count < 10200 and count >= 200:
        data['level_venture'] = '2'   # 200-10000 分为2类
    else:
        data['level_venture'] = '3'   # 其余为3类

    if data['e_address'] in ad_address.keys():    # 若e_address字段含有省份： 设置 ad_id和e_address字段的值。
    ...
    if data['m_address'] in ad_address.keys():   # 若m_address字段含有省份： 设置m_address字段的值。
        data['m_address'] = ad_address[data['m_address']]

    ad_class_value = data['ad_category'].split(';')[0]   # 广告内容标签
